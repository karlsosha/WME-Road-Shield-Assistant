...
function rsaInit() {
  ...
  console.log(
    `SDK v ${sdk.getSDKVersion()} on ${sdk.getWMEVersion()} initialized`,
  );
...
<b>KNOWN ISSUES:</b><br>
  ...
  const RoadAbbr: CountryRoadInfo = {
    ...
    40: {
      ...
      "British Columbia": {
        ...
        "Hwy ([4-9]|[1-9]d|10d|11[0-2])\b": 5001, // 5001: BC - Provincial Hwy
      ...
      Ontario: {
        ...
        "^Regional (Road|Rd) [1-9][0-9]{0,2}\\b": new Set<number>([
          5065, 5063, 5077,
        ]), // Ontario Regional
      },
      Quebec: {
    ...
    235: {
      ...
      Texas: {
        "^I-[1-9][0-9]{0,2}[A-Z]+\\s": new Set<number>([5, 2206]),
        "^SH-[1-9][0-9]{0,3}": new Set<number>([2117, 2123]),
        "^[Ss][Pp][Uu][Rr] [1-9][0-9]{0,3}\\b": 2126,
        "\\b[Ll][Oo][Oo][Pp] [1-9][0-9]{0,3}\\b": 2122,
        "^SR-[1-9][0-9]{0,3}": 2117,
        "^FM-[1-9][0-9]{0,3}": 2121,
  ...
  const Strings: LocalizedSettings = {
    en: {
      ...
      titleCase:
        "Segments/nodes with direction not in large-and-small-caps format",
      TitleCaseClr:
        "Segments/nodes with direction not in large-and-small-caps format",
      TitleCaseSftClr:
        "Direction in free text might not be in large-and-small-caps format",
    ...
    uk: {
      ...
      SegShieldErrorClr:
        "Сегменти, які мають шильди, але можливо вони непотрібні",
    ...
    fr: {
      ...
      SegShieldError:
        "Segments ayant un cartouche mais ne devraient peut-être pas",
      SegShieldErrorClr:
        "Segments ayant un cartouche mais ne devraient peut-être pas",
  ...
  function applyDirectionLabel(properties: FeatureProperties): boolean {
  ...
  async function loadSettings() {
    const localSettings = JSON.parse(
      <string>localStorage.getItem("RSA_Settings"),
    );
    const serverSettings =
      await WazeWrap.Remote.RetrieveSettings("RSA_Settings");
    if (!serverSettings) {
  ...
  async function saveSettings() {
    ...
    const serverSave = await WazeWrap.Remote.SaveSettings(
      "RSA_Settings",
      localSettings,
    );
      ...
      for (let i = 0; i < BadNames.length; i++) {
        ...
        if (BadNames[i]) {
          ...
          for (let s in turnGuid.roadShields) {
            turnGuid.roadShields[s].direction = fixName(
              turnGuid.roadShields[s].direction,
            );
          }
          ...
          W.model.actionManager.add(
            new SetTurn(turnGraph, turn.withTurnData(turnDat)),
          );
        }
      }
    });
  ...
  function processSeg(seg: Segment) {
    ...
    let city: City | null = sdk.DataModel.Cities.getById({
    ...
    let candidate: Candidate = isSegmentCandidate(seg, stateName, countryID);
  ...
  function isStreetCandidate(
    street: Street | null,
    stateName: string | null,
    countryId: number | null | Set<number>,
  ): Candidate {
  ...
  function matchTitleCase(street: Street) {
    ...
    if (dir !== "" && dir !== null) {
      ...
      if (
        dir.match(
          /([ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘʀꜱᴛᴜᴠᴡʏᴢ][a-z]|[a-z][ᴀʙᴄᴅᴇꜰɢʜɪᴊᴋʟᴍɴᴏᴘʀꜱᴛᴜᴠᴡʏᴢ])/,
        ) != null
      )
        isBad = true;
  ...
  function displaySegShields(
    ...
    $.each(segment.geometry.coordinates, function (idx, param) {
      ...
      if (oldparam.x !== null && oldparam.y !== null) {
        if (
          Math.abs(oldparam.x - param[0]) > labelDis.space ||
  ...
  function labelDistance(): LabelDistance {
    ...
    let label_distance: LabelDistance = {
      icon: 0,
      label: 0,
      space: 0,
    };
